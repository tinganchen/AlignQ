...
class LambdaLayer(nn.Module):
    ...
    def forward(self, x):
        return self.lambd(x)
'''
def conv3x3(wbit, abit, stage, in_planes, out_planes, stride=1, groups=1, dilation=1):
...
def conv1x1(wbit, abit, stage, in_planes, out_planes, stride=1):
    """1x1 convolution"""
    Conv2d = conv2d_Q_fn(w_bit = wbit, a_bit = abit, stage = stage)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, wbit, abit, stage, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv3x3(wbit, abit, stage, inplanes, planes, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(wbit, abit, stage, planes, planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, wbit, abit, stage, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv1x1(wbit, abit, stage, inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(wbit, abit, stage, width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(wbit, abit, stage, width, planes * self.expansion)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.wbit, self.abit, self.stage, self.inplanes, planes * block.expansion, stride),
