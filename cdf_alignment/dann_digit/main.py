...
def main():
    ...
    src_loader = import_module(f'data.{args.src_data}').__dict__['Data'](args)
    tgt_loader = import_module(f'data.{args.tgt_data}').__dict__['Data'](args)
    ...
    if args.method == 'ours':
        ...
        ARCH = f'dann_{args.method}'  

    # load training model

    #model_t = import_module(f'model.{args.arch}').__dict__[args.model](stage = args.stage, wbit = args.bitW, abit = args.abitW).to(device)
    model_t = import_module(f'model.{ARCH}').__dict__[args.model](stage = args.stage, wbit = args.bitW, abit = args.abitW).to(device)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if args.tgt_split:
            ...
            tgt_test_prec1, tgt_test_prec5, tgt_test_prec1_domain = test(args, tgt_data_loader, model_t, epoch, 'target')
        
        if args.src_split:
            src_test_prec1, src_test_prec5, src_test_prec1_domain = test(args, src_data_loader_eval, model_t, epoch, 'source')
        else:
            src_test_prec1, src_test_prec5, src_test_prec1_domain = test(args, src_data_loader, model_t, epoch, 'source')
...
def train(args, src_data_loader, tgt_data_loader, models, optimizers, epoch):
    ...
    for i, ((images_src, class_src), (images_tgt, _)) in data_zip:
        ...
        _, tgt_domain_output = model_t(images_tgt, alpha=alpha)
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(loader_test, 1):
            ...
            if flag == 'target':
                ...
                labels_domain = torch.zeros(inputs.size(0)).long().to(device).to(device)
