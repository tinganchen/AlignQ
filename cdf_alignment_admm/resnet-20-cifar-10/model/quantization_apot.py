...
class weight_quantize_fn(nn.Module):
    def __init__(self, w_bit, stage, power=True):
        ...
        self.register_parameter('wgt_alpha', nn.Parameter(torch.tensor(3.0)))
...
def activation_quantize_fn(a_bit, stage, grid, power=True):
    b = a_bit
...
def conv2d_Q_fn(w_bit, a_bit, stage):
    class QuantConv2d(nn.Conv2d):
        def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
            ...
            self.bit = w_bit # 4
            self.weight_quant = weight_quantize_fn(w_bit=self.bit, stage=stage, power=True)
            self.act_grid = build_power_value(self.bit, additive=True)
            self.act_alq = activation_quantize_fn(self.bit, stage, self.act_grid, power=True)
